<template>
    <v-card outlined>
        <v-card-title>
            Screen # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <Number label="SeatingCapacity" v-model="item.seatingCapacity" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/screens/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
