...
public class Screen {
    ...
    public static void reduceSeating(MovieReserved movieReserved) {
         ...
         */
        repository().findById(Long.valueOf(movieReserved.getScreenId())).ifPresent(screen -> {
            screen.setSeatingRemain(screen.getSeatingRemain() - movieReserved.getSeatingQty());
            repository().save(screen);
        });
    ...
    public static void addSeating(ReserveCanceled reserveCanceled) {
         ...
         * Example 2: finding and process
         */
        repository().findById(Long.valueOf(reserveCanceled.getScreenId())).ifPresent(screen -> {
            screen.setSeatingRemain(screen.getSeatingRemain() + reserveCanceled.getSeatingQty());
            repository().save(screen);
        });
